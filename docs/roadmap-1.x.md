...
- 用户选择 C6“最小中性过渡”。依据 Next.js `template` 的重置语义，不使用会重置筛选状态的 template 或伪造 route exit；保留 Navbar、背景与滚动的既有职责，仅让路由内容以 240ms 淡入和 6px 纵移进入。
- Blog、Knowledge 列表及详情、Projects 详情和 About 使用同一 `PageTransition`；reduce 模式即时显示。用户已手动验证跨板块、同板块文章、前后退、刷新/hash 与快速导航，确认无空白、叠页或滚动接管问题。
- lint、TypeScript、根路径与 Pages 子路径静态构建、smoke 和图片校验通过。未推送、未部署。
...
| 编号 | 选择 | 候选与边界 |
| --- | --- | --- |
| C1 | 阅读玻璃承托 | **已完成：**Blog/Knowledge 阅读面板深浅主题承托分别提高到 0.68/0.72，保留模糊边缘；不增加阅读模式开关。已与 C2 分轮修改，用户确认视觉无异常 |
| C2 | 正文字号 | **已确认：**共享正文保留 18px/31.5px；用户实测认为阅读舒服 |
| C3 | 代码主题 | **已选、无需改动：**深浅都固定暗色；行号/focus 基线已在 R7/R9 处理 |
| C4 | 中文引用 | **已完成：**Blog 保留斜体，Knowledge 改正体；不建完整 callout |
...
- 手机复测仍见 Blog/Knowledge 同板块返回和列表刷新时背景短暂右移，电脑端正常；`lvh` 固定高度不足以消除该问题。窄视口静态预览测得 `innerWidth=390px` 时全局稳定滚动条槽让背景容器仅有 375px，居中放大的图片因而存在横向裁切变化；手机断点取消根元素的稳定槽，仅背景固定层使用完整视口宽度。另将不需要滚动模糊的常驻背景改为原生静态图片，跨板块新图滑入仍保留。390px 预览连续刷新与 Blog/Knowledge 文章返回时背景容器均保持 390px、图片左边界不变；真实手机仍待用户复测，不把未复测结果标成完成。
- 项目详情页目测反馈“返回项目”呈整行长条：按钮原是 `PageTransition` 纵向 flex 容器的直接子项，被交叉轴拉伸。将按钮移出内容入场层，恢复随文字收缩的宽度，同时保留 44px 触控高度。根路径静态预览在 1280px 和 390px 视口测得按钮均约 107px 宽、44px 高；未改项目正文动效。
- 继续核对 R15：390px 静态预览中 Blog 标签默认只显示前 4 个，展开后显示全部 17 个；C5 在 MDX 测试帖正文链接实际呈现 1px 渐变线，标题锚点仍沿用独立样式；Blog 引用计算样式为斜体，Knowledge 引用为正体。上述是实现验证，C1/C4/C5/C7 的最终视觉感受仍由用户目测确认。

### R15 收尾验收（2026-09-18）

- 用户复查后反馈“没什么问题”，本轮选定视觉及此前手机背景、返回按钮反馈视为通过；C6 属于 R14，不作为 R15 待办。先前“待验收”“待真机复测”记录保留为过程记录，以本节最终结论为准。
- 静态预览截图对照了同一 Blog 测试文章的深色/浅色、1280px/390px 首屏，以及 Knowledge 长文的深色/浅色、390px 首屏；检查了引用体例、18px/31.5px 正文、玻璃承托、手机标签入口与文章返回按钮。键盘 Tab 焦点可见；标签展开具有 `aria-expanded`，复制代码有反馈，手机交互目标至少 44px；路由动效沿用 `useMotionPolicy` 的减少动效分支。抽查浏览器无错误日志。
