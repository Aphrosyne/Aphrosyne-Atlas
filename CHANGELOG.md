...
- 修复 GitHub Pages 静态 smoke 检查仍引用已删除或重命名文章 slug 的问题；文章与项目详情路由现在从构建生成的 `search-index.json` 自动纳入检查，新增、删除或重命名内容不再需要手动修改 smoke 清单。
- 更新文章渲染审计与 UI/UX 审计中的旧 Knowledge 路径引用，改为当前短 slug。
...
- Blog 与 Knowledge 现支持经过 schema 校验的 `pinned` 布尔字段；置顶的公开内容会在各自列表优先显示、标注“置顶”，并使用 2px 旋转渐变描边。所有既有文章都显式声明 `pinned: false`，便于日后单篇启用。
- 站点公开联系方式新增 Bilibili 主页入口。

### Changed

- Blog 与 Knowledge 置顶卡片保留原有玻璃填充；置顶 Blog 卡片不再执行上浮动效，避免渐变描边在合成时闪烁。系统设置为减少动效时，描边保持静态。
- 知识库使用说明新增置顶字段的适用范围、启用方式与发布状态限制。
...
- 页面导航统一为短淡入与轻微纵移的可靠进入过渡；不伪造退出动画、不重置筛选状态，减少动态效果时即时显示。

### Fixed

- 内容 schema 与 canonical index 模块现在在 `src/lib` 作用域明确按 ESM 解析，构建不再重复输出 `MODULE_TYPELESS_PACKAGE_JSON` 警告。
