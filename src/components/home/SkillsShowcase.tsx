    ...
    <div className="group relative w-full overflow-hidden rounded-xl border border-border/50 bg-surface/30 backdrop-blur-xl p-5 transition-all duration-300 hover:border-border/70 hover:shadow-lg hover:shadow-accent/5">
      ...
      <div className="relative mt-3 flex flex-wrap gap-2">
        {skills.map((skill) => (
          <span
            key={skill}
            className="inline-flex items-center rounded-full border border-border/40 bg-surface/40 px-3 py-1 text-xs text-muted/70 transition-all duration-200 hover:border-accent/20 hover:bg-accent/[0.04] hover:text-fg/90"
          >
            {skill}
          </span>
        ))}
