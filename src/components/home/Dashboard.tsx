...
function NowWidget() {
  ...
  useEffect(() => {
    fetch('https://api.github.com/users/aphrosyne/repos?sort=updated&per_page=2')
      .then(r => r.json())
      .then(data => {
  ...
  return (
    <Card direction="right" className="flex flex-col gap-3">
      <div className="text-[10px] text-white/40 tracking-widest uppercase">Now</div>
        ...
        <div className="space-y-2">
          {repos.map((r) => (
            <a
              ...
              className="block rounded-lg hover:bg-surface/30 transition-colors p-1.5 -m-1.5 group"
            >
              <div className="flex items-center gap-1.5">
                <svg className="w-3 h-3 text-white/50 group-hover:text-white/70" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z"/></svg>
      ...
      {/* Daily quote */}
      {quote && (
        <div className="text-xs text-white/50 leading-relaxed italic">
          「{quote.hitokoto}」
        </div>
      )}
    </Card>
...
export default function Dashboard({ siteName, recentPosts, projects }: DashboardProps) {
  return (
    <section id="content" className="px-4 pb-20 max-w-5xl mx-auto">
      ...
      >
        {/* Row 1: Bio + Recent Posts */}
        <div className="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-4 lg:gap-5">
          {/* Bio */}
          ...
          <Card direction="left">
            ...
            {recentPosts.map((p) => (
              <Link key={p.slug} href={`/blog/${p.slug}`}
                className="flex items-center justify-between py-2 px-1.5 rounded-lg -mx-1.5 transition-colors hover:bg-surface/40 group"
              >
                <span className="text-sm font-medium text-white/80 truncate group-hover:text-accent transition-colors">{p.title}</span>
                <span className="text-[11px] text-white/40 shrink-0 ml-4">{p.date}</span>
              </Link>
            ))}
          </Card>
        </div>
        ...
        {/* Row 2: Projects + Now */}
        <div className="grid grid-cols-1 md:grid-cols-[1fr_220px] gap-4 lg:gap-5">
