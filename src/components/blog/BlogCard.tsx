...
export default function BlogCard({ slug, title, date, excerpt, tags, index, onTagClick }: BlogCardProps) {
  ...
  return (
    ...
    >
      <Link
        href={`/blog/${slug}`}
        className="group block rounded-2xl bg-surface/50 backdrop-blur-sm border border-border/40 p-5 transition-shadow duration-300 hover:shadow-[0_14px_30px_rgba(0,0,0,0.12)]"
      >
