...
function pinnedState(data, filePath, publication) {
  if (data.pinned === undefined) return false
  if (typeof data.pinned !== 'boolean') fail(filePath, 'frontmatter.pinned 必须是布尔值')
  if (data.pinned && publication !== 'published') fail(filePath, '只有 published 内容可以置顶')
  return data.pinned
}
...
export function parseBlogMetadata(data, filePath, slug) {
  ...
  return { slug, title: requiredString(data, 'title', filePath), date: dateString(data.date, 'date', filePath), tags: stringArray(data, 'tags', filePath), excerpt: requiredString(data, 'excerpt', filePath), publication, pinned: pinnedState(data, filePath, publication), readingTime }
}
...
export function parseKnowledgeMetadata(data, filePath, slug) {
  ...
  return { slug, title: requiredString(data, 'title', filePath), type, status, publication, pinned: pinnedState(data, filePath, publication), excerpt: requiredString(data, 'excerpt', filePath), tags: stringArray(data, 'tags', filePath), gameVersion: data.game_version === undefined ? undefined : requiredString(data, 'game_version', filePath), lastEdited: dateString(data.last_edited, 'last_edited', filePath), related: stringArray(data, 'related', filePath), sources: sources.map((source) => ({ label: source.label.trim(), href: source.href })) }
}
